advancement revoke @s only teleport_book:teleport_book/writable/mainhand

execute store result storage teleport_book:teleport data.id int 1 run scoreboard players get @s utils.player.id
data modify storage teleport_book:teleport data.player set from entity @s SelectedItem.components."minecraft:writable_book_content".pages[0].raw
