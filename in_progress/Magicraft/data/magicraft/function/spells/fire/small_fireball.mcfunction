...
execute anchored eyes positioned ^ ^ ^.5 run summon small_fireball ~ ~ ~ {acceleration_power:0.1d,Tags:["magicraft.spell.fire.small_fireball"]}

data modify entity @n[tag=magicraft.spell.fire.small_fireball] Owner set from entity @s UUID

data modify entity @n[tag=magicraft.spell.fire.small_fireball] Motion set from storage utils:motion motion
